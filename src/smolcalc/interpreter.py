...
class Interpreter:
    ...
    def visit_plus_node(self, node) -> Number:
        return self.visit(node.node)

    def visit_minus_node(self, node) -> Number:
        return Number(operator.neg(self.visit(node.node).value))

    def visit_exponent_node(self, node) -> Number:
    ...
    def visit_square_root_node(self, node) -> Number:
        value = self.visit(node.node).value
        if value < 0:
            self.raise_error_complex_numbers()

        return Number(math.pow(value, 0.5))

    def visit_nlog_node(self, node) -> Number:
        value = self.visit(node.node).value
        if value <= 0:
            self.raise_error_complex_numbers()
    ...
    def visit_log_10_node(self, node) -> Number:
        value = self.visit(node.node).value
        if value <= 0:
            self.raise_error_complex_numbers()
    ...
    def visit_factorial_node(self, node) -> Number:
        value = self.visit(node.node).value
        ...
        return Number(math.gamma(value + int(not self.special)))
